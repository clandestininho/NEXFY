...
const fileToGenerativePart = async (file: File): Promise<Part> => {
  const base64EncodedData = await new Promise<string>((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      // Performance optimization: substring is significantly more CPU/memory efficient than split
      // for large strings like Data URLs (e.g., 5MB+ images) as it avoids intermediate array allocations.
