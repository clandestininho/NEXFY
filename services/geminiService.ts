...
const fileToGenerativePart = async (file: File): Promise<Part> => {
  const base64EncodedData = await new Promise<string>((resolve, reject) => {
    ...
    // ⚡ Bolt: Using substring instead of split avoids intermediate array and string allocations, making base64 extraction significantly more CPU and memory efficient for large files.
    reader.onload = () => {
