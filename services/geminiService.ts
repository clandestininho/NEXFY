...
const fileToGenerativePart = async (file: File): Promise<Part> => {
  const base64EncodedData = await new Promise<string>((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      // ⚡ Bolt: Use substring instead of split to avoid large intermediate array allocations
      // Reduces memory overhead and is significantly faster for large image files
