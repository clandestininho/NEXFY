...
const fileToGenerativePart = async (file: File): Promise<Part> => {
  const base64EncodedData = await new Promise<string>((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      // Extracting base64 payload via substring instead of split avoids intermediate array
      // and string allocations, making it significantly more memory/CPU efficient for large files.
